...
let append_js2 = "function gz_copy_refr(){let gz_btns=document.getElementsByClassName('gz_cls_copy_button');let gz_f_c=function(){let gz_attr=this.getAttribute('id');gm_clipboard(gz_attr);};for(let i=0;i<gz_btns.length;i++){console.log(gz_btns[i]);gz_btns[i].addEventListener('click',gz_f_c,false);}}";
